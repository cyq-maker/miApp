...
/* 
 * queryURLParams:获取url地址问号和参数信息（可能也包括hash值）
 */
...
function _each(obj, callback, context = window) {
    ...
    if (isLikeArray) {
        ...
        for (let i = 0; i < arr.length; i++) {
            let item = arr[i],
                //this对象是context的值，回调函数的参数是item和i
...
/* 
 * _throttle:函数的节流，达到一定的时间间隔或者频率就会执行一次
 * @param func 传入函数，最后一个参数是额外增加的this对象，.apply(this, args) 这种方式，this无法传递进函数
 * @param wait 标识时间窗口的间隔
 */
...
/* 
 * _debounce:函数的防抖，将若干函数调用合成为一次，并在给定时间过去之后，或者连续事件完全触发完成之后，调用一次
 */
